...
class MdTopAppBar extends HTMLElement {
 ...
 getContent() {
  return html`
   ...
   <span id="acciones">
    <slot name="action" onslotchange="${this.configuraAction}"></slot>
   </span>`.htmlTxt
 }
  ...
  if (idAdicional !== "") {
   ...
   if (this._adicional !== null) {
    if (this.classList.contains("apple")) {
     this._adicional.style.top = "env(titlebar-area-height, 48px)"
    } else {
     this._adicional.style.top = "env(titlebar-area-height, 64px)"
    }
   }
  }
 }
